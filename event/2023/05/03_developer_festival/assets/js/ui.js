const mapList = [
  ...
  },
  {
    ...
    vote: 26989,
    acceptance_speech:
      "거미인간처럼 슝슝 날아다니는 맵을<br/>  만들고 싶었습니다. <br/> 좋아해 주시고 투표해 주셔서<br/> 정말 무한히 엄청나게 감사합니다. ",
  },
  {
    id: "04",
    name: "차사 훈련소",
...
재미를 많이들 얻었으면 좋겠습니다. <br/><br/> 다음에 또 기회가 된다면<br/>다시 이상한 맵으로 찾아뵙겠습니다.<br/> 재미있게 즐겨주셔서 감사합니다. `,
  ...
  },
  {
    ...
    src: "item-more-09",
    description:
      "바닥이 사라졌다 생겼다~ 떨어지지 않고 레브 요원을 피해 달려보세욧!",
    map_type: "하드코어",
    difficulty: "★★★",
    game_type: "노템전/아이템전/8인/30인",
    vote: 21991,
...
그리고 투표해 주신 런너분들께도 GRAND 감사의 인사를 올립니다.<br/><br/> 여러분의 표 덕분에 이렇게 인사드릴 기회를 얻게 되어 기쁘네요<br/> <br/>평소에도 런너분들 곁에서 달리고 있지만,<br/> 개발자 이벤트 때는 누가 우리 아이템을 <br/>착용하고 있진 않은지 낚시터를 돌아다니며 <br/> 남몰래 런너분들을 구경하곤 한답니다.<br/> 런너분들 사랑해요♡♡♡`,
  ...
  },
  {
    ...
    description:
      "노곤노곤한 날 따뜻한 햇볕 아래 나무 휴식공간.<br/> 귀여운 뺙뺙이와 함께라면 잠시나마 피로함이 날아갈지도?",
    position: "모션 아이템",
    ref: `스토리 캐릭터 포함 <br/> 모든 캐릭터 모션 적용`,
    vote: 13727,
  },
  {
    ...
    description:
      "인형 뽑기 기계 속에 갇혔어요!<br/> 기다리고 있을 테니<br/> 언릉 달려와 뽑아 주세요~",
    position: "모션 아이템",
    ref: `스토리 캐릭터 포함 <br/> 모든 캐릭터 모션 적용`,
    vote: 17142,
  },
...
$(function () {
  ...
  section03ItemMapping(
    ".result-wrap .section03-content-items",
    false,
    itemList
  );
...
function popupItem01(
  ...
  $popup.find(".popup-item").empty().append(`
      <div class="content-top">
        ...
        <div class="item-img">
        <img src="//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/popup-item/${src}.png" alt={name}>
        </div>
...
function userTypeChange(id, typeNum) {
  ...
  switch (type) {
    case 1:
      return $(".user-state .state-id .user-set").text(
        "로그인 후 참여하실 수 있습니다."
      );
    case 2:
      return $(".user-state .state-id .user-set").text(
...
function section02ItemMapping(selector, random, mapList) {
  ...
  if (random) {
    mapList
      ...
      <label class="item-box">
        ...
        <span class="item-img">
      <img  src="//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/section02-item/item-${item.id}.png" alt="${item.name}">  </span>
  ...
  `;
    ...
    mapList
      ...
      <div class="item-box">
        <span class="item-img">
      <img  src="//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/section02-item/item-${
        item.id
      }.png" alt="${item.name}">  </span>
        <span class="text">${item.name}</span>
        <div class="votes-number"><span class="heart-icon"></span> ${item.vote.toLocaleString(
          "ko-KR"
        )}</div>
        <button type="button" class="btn btn-caution movie-btn" onclick="popupItem01('[data-popup-id=item]','${
          item.name
        }','${item.src}','${item.description}','${item.map_type}','${
          item.difficulty
        }','${item.game_type}')">
          <span class="hide">자세히보기</span>
        </button>
...
function section03ItemMapping(selector, random, itemList) {
  ...
  if (random) {
    itemList
      ...
      <label class="item-box">
        ...
        <span class="item-img">
        <img  src="//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/section03-item/item-${item.id}.png" alt="${item.name}">  </span>
        <span class="text">${item.name}</span>
  ...
  `;
    ...
    itemList
      ...
      <div class="item-box">
        <span class="item-img">
        <img  src="//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/section03-item/item-${
          item.id
        }-result.png" alt="${item.name}">  </span>
        <span class="text">${item.name}</span>
        <div class="votes-number"><span class="heart-icon"></span> ${item.vote.toLocaleString(
          "ko-KR"
        )}</div>
        <button type="button" class="btn btn-caution movie-btn" onclick="popupItem02('[data-popup-id=item02]','${
          item.name
        }','${item.src}','${item.description}','${item.position}','${
          item.ref
        }')">
          <span class="hide">자세히보기</span>
        </button>
...
function onTopClickBoxButton(thisButton) {
  ...
  if (thisButton === "vote") {
    header.style.cssText =
      "background: url(//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/bg_header.jpg) no-repeat center top;height: 730px;";
    voteWrap.style.display = "block";
    resultWrap.style.display = "none";
  } else {
    header.style.cssText =
      "background: url(//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/bg_header02.jpg) no-repeat center top;height: 700px;";
...
let images = [];
let imagePaths = [
  "//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/bg_header02.jpg",
  "//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/bg_section02_result.jpg",
  "//tr-image.game.onstove.com/event/202305/17_developerFestival02/assets/images/bg_section03_result.jpg",
];
...
function fnGetItem(itemtype, votenum) {
  if (itemtype == "mapnum") {
    if ($("input:checkbox[name=mapnum]:checked").length == 0) {
      alert("请选择要投票的地图。");
      isClickChkSet();
      return;
    }
    $("input:checkbox[name=mapnum]").each(function (index) {
      if ($(this).is(":checked") == true) {
        votenum = $("input:checkbox[name=mapnum]")[index].value;
      }
    });
    // do ajax
    alert("投票成功");
    document.location.reload();
  } else if (itemtype == "itemnum") {
    if ($("input:checkbox[name=itemnum]:checked").length == 0) {
      alert("请选择想要投票的单品。");
      isClickChkSet();
      return;
    }
    $("input:checkbox[name=itemnum]").each(function (index) {
      if ($(this).is(":checked") == true) {
        votenum = $("input:checkbox[name=itemnum]")[index].value;
      }
    });
    // do ajax
    alert("投票成功");
